...
export const EncryptedAttachment = ({ msg, isMe }) => {
  ...
  return (
    <div 
      ...
      className="flex items-center gap-3 p-3 hover:bg-black/10 transition-colors cursor-pointer"
    >
      <div className={`p-2 rounded-lg ${isMe ? 'bg-black text-white' : 'bg-white text-black'}`}>
        <FileText size={18} />
      </div>
      ...
      {decryptedUrl && <Download size={16} className="text-gray-400" />}
    </div>
  );
};
